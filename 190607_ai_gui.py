...
if __name__ == "__main__":
    try:
        ...
        if '월9시-' and '11' and '17' in list_combined[0]+lecture_room_row_fixed[0]:
            print("yes")
    ...
    for i in range(0,20):
        ...
        # 로네임을 고정시킴
        for i in range(len(timeslot_col_fixed)):
            # 엑셀 데이터에서 로 네임과 일치되는 것을 찾음
            for j in range(len(your_list)):
                print("여기인가요?")
                if list_combined[i]==(your_list[j][0]+your_list[j][1]):
                    # x축 값이 계속 바뀜, idx도 계속 증가
                    for room_idx, room in enumerate(lecture_room_row_fixed):
                        # x축(컬럼네임)이 해당되는 게 있을 때 증가된 idx 반영, 처음 설정한 row idx 값 그대로 사용
                        if your_list[j][2] == room:
                            # print(room_idx + 2)
                            table.cell(i, room_idx+2, your_list[j][3])


                # if your_list[i][2] == room:
                #     print(room_idx)
    ...
    #             if your_list[i][0] + your_list[i][1] == list_combined[0]:
    #                 if your_list[i][2] == lecture_room_row_fixed[room_idx]:
    ...
    # for i in range(len(your_list)):
    #     if your_list[i][0] + your_list[i][1] in list_combined[0]:
    #         if '2117호' == lecture_room_row_fixed[0]:
